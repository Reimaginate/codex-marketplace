...
Design Hub TOC item IDs are not separate navigation IDs. TOC item `type` values should use canonical casing: `Page` or `Group`.

- For page TOC items, use `type:"Page"`. The `id` must equal the created or existing page `id`, and `linkedSectionId` must also equal that same page `id`.
- For new group TOC items, use `type:"Group"` and do not supply `id`; group IDs are generated by the server.
- For existing group TOC items returned by Design Hub, treat the returned `id` as the group ID. A group does not normally have `linkedSectionId`.
- When adding anything under a group, use the server-returned group TOC item `id` as `parentId`.
- Do not invent a different TOC item ID for a page or group. Do not create page TOC items where `id` and `linkedSectionId` differ.
...
- Use a TOC item shaped as `type:"Group"` and `label:<requested group name>`. Do not include `id`; use the server-returned item `id` as the group ID after insertion.
